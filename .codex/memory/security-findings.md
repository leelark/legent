...
| Service identity | Internal credential guards remain the baseline; audience-resolution chunk reads and deliverability suppression internal history/bulk-check routes now add signed caller/action/tenant/workspace headers with service allowlists, but full platform identity, mTLS, and coverage across all internal routes are still pending. | future service identity expansion / release evidence | Extend signed identity or service JWT to remaining internal content/import/data-extension routes, then collect target mTLS/NetworkPolicy evidence. |
