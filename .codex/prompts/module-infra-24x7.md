...
Late-join rule: this module thread may start after the coordinator. Do not treat earlier coordinator planning as a blocker. Only an active overlapping source-code/infra lease, failed validation in this module scope, missing external evidence, credentials, production access, or explicit human decision can block implementation. If a shared .codex metadata lease is active, continue infra/source work and defer only the metadata write/handoff until that exact lease clears.
