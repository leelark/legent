...
function isWorkspaceOptionalEndpoint(url: string | undefined): boolean {
  ...
  return /\/api\/v1\/(?:users\/preferences|admin\/(?:bootstrap|branding|configs|contact-requests|operations|public-content|settings)|core|platform\/(?:notifications|search|webhooks))(?:\/|$)/.test(url);
}
