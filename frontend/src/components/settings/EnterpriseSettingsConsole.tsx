...
export function EnterpriseSettingsConsole({ initialSection = 'profile' }: { initialSection?: SettingsSection }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (domainRes.status === 'fulfilled') {
        ...
        setDomains(records);
        setSelectedDomain((prev) => prev || asText(records[0]?.domainName || records[0]?.domain_name));
      }
    } finally {
      setLoading(false);
    }
  ...
  const summary = useMemo(() => {
    const verified = domains.filter((domain) => domain.status === 'VERIFIED' || domain.isActive).length;
    const alerts = domains.filter((domain) => !(domain.spfVerified && domain.dkimVerified && domain.dmarcVerified)).length;
    return {
    ...
    <div className="space-y-5">
      <section className="overflow-hidden rounded-lg border border-border-default bg-surface-elevated shadow-[0_18px_70px_rgba(14,116,144,0.10)]">
        <div className="relative grid gap-6 p-5 lg:grid-cols-[minmax(0,1fr)_380px] lg:p-7">
          <div className="pointer-events-none absolute inset-0 [background:radial-gradient(circle_at_12%_12%,rgba(14,165,233,0.14),transparent_28%),radial-gradient(circle_at_90%_20%,rgba(16,185,129,0.13),transparent_30%)]" />
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
        <section className="rounded-lg border border-border-default bg-surface-elevated p-5 shadow-sm">
          ...
          <div className="mt-5 overflow-hidden rounded-lg border border-border-default">
            <div className="grid grid-cols-[minmax(160px,1fr)_80px_80px_80px_170px] gap-0 bg-surface-secondary px-3 py-2 text-xs font-semibold text-content-muted">
              <span>Domain</span>
              <span>SPF</span>
              <span>DKIM</span>
              <span>DMARC</span>
            ...
            <div className="divide-y divide-border-default">
              {domains.map((domain, index) => {
                const name = asText(domain.domainName || domain.domain_name, `domain-${index}`);
                return (
                  <div key={`${asText(domain.id, name)}-${index}`} className="grid grid-cols-[minmax(160px,1fr)_80px_80px_80px_170px] items-center gap-0 px-3 py-3 text-sm">
                    ...
                    <DnsIcon ok={Boolean(domain.spfVerified)} />
                    <DnsIcon ok={Boolean(domain.dkimVerified)} />
                    <DnsIcon ok={Boolean(domain.dmarcVerified)} />
                    <div className="flex justify-end gap-2">
                      <Button variant="secondary" size="sm" onClick={() => setSelectedDomain(name)}>View</Button>
                      <Button variant="outline" size="sm" onClick={() => verifyDomain(asText(domain.id))}>Verify</Button>
                    </div>
                  </div>
                );
              })}
