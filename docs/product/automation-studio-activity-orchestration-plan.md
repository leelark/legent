...
Automation Studio currently exposes activity types `SQL_QUERY`, `FILE_DROP`, `IMPORT`, `EXTRACT`, `SCRIPT`, `WEBHOOK`, and `NOTIFICATION`. `SQL_QUERY`, `IMPORT`, `WEBHOOK`, and `NOTIFICATION` have supported live execution, and live execution requires an active activity plus explicit `confirmLiveRun=true`. Empty run requests default to dry run; file-drop/extract stay validation-only; script execution remains blocked.
...
| 6 | `automation-send-activity-handoff` | BACKLOG | CAMPAIGN_SERVICE_OWNER | Add governed send activity handoff that preserves campaign policy, suppression, warmup, rate, inbox-safety, idempotency, and explicit live confirmation. | Automation/campaign/delivery tests and send safety gates. |
