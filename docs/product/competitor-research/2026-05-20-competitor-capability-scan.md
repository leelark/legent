...
Competitor parity is converging around marketer-owned visual journeys, guided setup, dynamic behavioral segmentation, ecommerce and event triggers, cross-channel steps, predictive audience signals, step-level analytics, experimentation, and deliverability-aware sending. As of the 2026-05-24 local queue reconciliation, Legent has closed the local runtime-node contract, flow-analytics/experimentation, mode-aware workflow, send-governance policy, contact-data governance, contact lifecycle audit, AI draft-application contract, and approved deterministic send-time schedule evidence slices. That evidence supports internal queue state only; it does not establish competitor parity, model-backed AI, production behavior, or deliverability/throughput outcomes.
...
|---|---|---|---|
| `segment-builder-v2-rule-taxonomy` | Define static, dynamic, behavioral, computed, nested, null, timezone, consent, suppression, and send-eligibility rule semantics before UI expansion. | Local segment and predictive governance exists; the 2026-05-23 taxonomy draft is documentation only and does not prove execution, governance UI, or parity. | Refresh segmentation competitor facts before public parity claims. |
| `segment-builder-v2-execution-plan` | Compile and evaluate segment rules with bounded reads, explain output, indexed execution, recompute scheduling, and failure handling. | DONE_LOCAL 2026-05-24: existing subscriber, list-membership, and segment-membership rule families compile through a bounded plan; count preview and a read-only execution-plan explain response expose metadata; recompute compiles before membership deletion and fail-closes unsupported relationship traversal. Indexed high-volume behavioral execution is still unproven. | Requires event/relationship execution design, backend tests, and performance evidence before throughput or parity claims. |
...
| `automation-live-file-movement-storage-adapter` | Move file-drop and extract activities from validation-only history to tenant-scoped object movement with ownership, hash, size, type, and retention proof. | DONE_LOCAL 2026-05-24: file-drop and artifact-backed extract movement run through a scoped storage adapter with idempotent live-run gates and redacted run history. Data-extension extract generation, target object-store drills, cleanup/retention proof, and production parity remain unproven. | Needs target evidence and source refresh before parity claims. |
...
| `ai-sto-runtime-scheduler` | Convert deterministic readiness checks into scheduled send-time decisions with confidence, fallback, and evidence. | DONE_LOCAL as of 2026-05-24: approved campaign-owned `SEND_TIME` snapshots can drive campaign scheduling and due-job dispatch with fail-closed evidence gates. This is not model-backed recipient-level STO. | Needs production-like evidence and model/provider architecture before optimization or parity claims. |
...
| `contact-builder-relationship-cardinality` | Model contact/data-extension relationships, cardinality, query preview, and admin governance separately from segmentation. | First-class relationship/cardinality schema contracts exist locally; query preview/explain UX, indexed execution, and parity evidence do not. | Requires further query-preview design, indexed execution proof, and source refresh before parity claims. |
...
| `contact-retention-deletion-audit` | Add retention, deletion, suppression, preference, and send eligibility audit history for Contact Builder style governance. | DONE_LOCAL 2026-05-24: tenant/workspace-scoped append-only audit captures subscriber deletion/merge/bulk-delete, suppression create/delete/recover/bulk-create, preference update/pause/unsubscribe/resubscribe, data-extension retention/delete/overwrite aggregates, and explicit send-eligibility checks using hashed email and bounded metadata. Target retention policy, production evidence, and public parity remain unproven. | Needs target audit-retention evidence and source refresh before parity claims. |
